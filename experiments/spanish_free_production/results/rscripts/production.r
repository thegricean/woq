...
setwd("d:/Dropbox/Tuebingen17SS/RA/woq/experiments/spanish_free_production/results")
# setwd("~/Dropbox/Tuebingen17SS/RA/woq/experiments/spanish_free_production/results")
# setwd("~/cogsci/projects/stanford/projects/woq/experiments/1c/results")
source("rscripts/helpers.r")

d = read.table(file="data/pilot.csv",sep=",", header=T, encoding="UTF-8")
d2 = read.table(file="data/pilot-addition.csv",sep=",", header=T, encoding="UTF-8")
d = rbind(d, d2)
# Those two were the invalid responses of only English and numbers.
d = d %>% filter(prolificID != "59f8565b680a95000122b101") %>% filter(prolificID != "57c07431a170cc000103ec15")
head(d)
nrow(d)
summary(d)
totalnrow = nrow(d)
